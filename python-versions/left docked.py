"""
DynamicIsland (Tkinter) — Slim Bottom-Left Widget
...
# —— anchor to BOTTOM-LEFT (new) ——
SNAP_LEFT_MARGIN_PX   = 8
SNAP_BOTTOM_MARGIN_PX = 8
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        # dragging (updated for bottom band)
    ...
    def _left_x(self):
        return int(round(SNAP_LEFT_MARGIN_PX * self.scale))
    def _bottom_y(self):
        sh = self.root.winfo_screenheight()
        return max(0, int(sh - self.height - round(SNAP_BOTTOM_MARGIN_PX * self.scale)))
    ...
    def _layout_resize(self):
        ...
        if final_w != self.width:
            self.width = final_w
            self.root.geometry(f"{final_w}x{self.height}+{self._left_x()}+{self._bottom_y()}")
    ...
    # ---------- drag / click-through (updated for bottom-left) ----------
    def _start_drag(self,e):
        # allow dragging while we're within the bottom band
        band_top = max(0, self._bottom_y() - ALLOW_DRAG_BAND_PX)
        if self.root.winfo_y() >= band_top:
    ...
    def _on_drag(self,e):
        ...
        # restrict y within bottom band
        band_top = max(0, self._bottom_y() - ALLOW_DRAG_BAND_PX)
        band_bot = self._bottom_y()
        y = self.root.winfo_y()+dy
        y = max(band_top, min(y, band_bot))
    ...
    def _stop_drag(self,_):
        if getattr(self,"_drag",False):
            ...
            self.root.geometry(f"+{self._left_x()}+{self._bottom_y()}")
